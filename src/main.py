...
def run_script(script, *args, cwd=None):
    ...
    output_text.insert(tk.END, f"> Running: {script} {' '.join(str(a) for a in args if str(a) != '')}\n")
    ...
    if getattr(sys, "frozen", False):
        ...
        ok = True
        try:
            sys.argv = [script_abs] + [str(a) for a in args if str(a) != ""]
            os.chdir(workdir)
            ...
            ok = e.code in (0, None)
            if not ok:
                buffer.write(f"{script} exited with status {e.code}\n")
        except Exception as e:
            ok = False
            buffer.write(f"Error running {script}: {e}\n")
        finally:
            sys.argv = old_argv
            os.chdir(old_cwd)
...
def install_deps():
    if getattr(sys, "frozen", False):
        output_text.insert(tk.END, "\nThis portable app already includes its dependencies.\n")
        output_text.see(tk.END)
        return
...
def run_helper_from_exe():
    if len(sys.argv) < 3:
        raise SystemExit("Missing helper script.")

    helper_script = Path(sys.argv[2])
    if not helper_script.is_file():
        raise SystemExit(f"Missing helper script: {helper_script}")

    sys.argv = [str(helper_script), *sys.argv[3:]]
    runpy.run_path(str(helper_script), run_name="__main__")


if __name__ == '__main__':
    if len(sys.argv) > 1 and sys.argv[1] == HELPER_MODE:
        run_helper_from_exe()
    else:
        create_gui()
